...
def run_to_task():
    ...
    predicted, representation = training_runners['sess'].run(
            [m.decoder_output,  m.encoder_endpoints[args.encoder_layer]], feed_dict={m.input_images: img} )

    if args.store_rep:
        s_name, file_extension = os.path.splitext(args.store_name)
        with open('{}.npy'.format(s_name + '_features' + ("_" + args.encoder_layer.replace("/", "_") if args.save_lname else "")), 'wb') as fp:
